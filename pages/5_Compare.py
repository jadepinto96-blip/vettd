...
.cmp-card { background:var(--surface); border:1px solid var(--border); border-radius:18px; padding:1.5rem;
...
.cmp-label { color:var(--text-3); } .cmp-val { color:var(--text-1); font-weight:600; }
</style>
...
<p style="font-size:15px;color:var(--text-3);max-width:520px;margin:0 auto;line-height:1.7;">Enter up to three creators and Vettd will score each, overlay them, and tell you who to pick.</p>
</div>
""", unsafe_allow_html=True)
...
for i, col in enumerate(cols):
    with col:
        ...
        st.markdown('<div style="font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--text-3);margin:.5rem 0 -.5rem;">Reel performance</div>', unsafe_allow_html=True)
...
if run:
    ...
    <div style="background:linear-gradient(135deg,rgba(124,58,237,.12),rgba(34,211,238,.08));
      ...
      <div>
        ...
        <div class="disp" style="font-size:26px;font-weight:700;color:var(--text-1);">{w['name']} <span style="color:var(--text-3);font-size:15px;font-weight:400;">{w['user']}</span></div>
        <div style="font-size:13px;color:var(--text-2);margin-top:6px;max-width:520px;line-height:1.6;">
    ...
    for i, (col, r) in enumerate(zip(rcols, results)):
        is_w = i == winner
        border = f"2px solid {accent[i]}" if is_w else "1px solid var(--border)"
    ...
    # ── RADAR OVERLAY ──
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown('<div style="font-size:11px;font-weight:700;letter-spacing:.15em;text-transform:uppercase;color:var(--text-4);margin-bottom:.5rem;">Signal overlay</div>', unsafe_allow_html=True)
    ...
    fig.update_layout(
        polar=dict(bgcolor="rgba(0,0,0,0)",
            radialaxis=dict(visible=True, range=[0,100], tickfont=dict(color="var(--text-4)", size=9), gridcolor="var(--border)", linecolor="var(--border)"),
            angularaxis=dict(tickfont=dict(color="var(--text-3)", size=11), gridcolor="var(--border)", linecolor="var(--border)")),
        height=420, margin=dict(t=30,b=30,l=60,r=60), paper_bgcolor="rgba(0,0,0,0)",
        legend=dict(font=dict(color="var(--text-2)"), bgcolor="rgba(0,0,0,0)", orientation="h", y=-0.1),
    )
    ...
    fig2.add_trace(go.Bar(
        ...
        textfont=dict(color="var(--text-2)", size=14),
    ))
    fig2.update_layout(
        title=dict(text="Vettd Score comparison", font=dict(color="var(--text-3)", size=12)),
        height=300, margin=dict(t=40,b=20,l=20,r=20), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        xaxis=dict(showgrid=False, color="var(--text-3)", tickfont=dict(color="var(--text-2)", size=12)),
        yaxis=dict(showgrid=True, gridcolor="var(--border)", range=[0,100], tickfont=dict(color="var(--text-4)")),
        bargap=0.5,
    )
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("""
    <div style="font-size:11px;font-weight:700;letter-spacing:.15em;text-transform:uppercase;color:var(--text-4);margin-bottom:.25rem;">★ Audience overlap</div>
    <div style="font-size:13px;color:var(--text-3);margin-bottom:1rem;max-width:640px;line-height:1.6;">
        ...
        with col:
            ...
            <div class="cmp-card" style="border-color:{vcolor}33;">
              <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;">
                ...
                <span style="font-size:12px;color:var(--text-2);font-weight:600;">{a['name']}</span>
                <span style="color:var(--text-4);font-size:12px;">↔</span>
                <span style="width:8px;height:8px;border-radius:50%;background:{accent[j]};"></span>
                <span style="font-size:12px;color:var(--text-2);font-weight:600;">{b['name']}</span>
              </div>
              <div class="disp" style="font-size:38px;font-weight:700;line-height:1;color:{vcolor};">{pct}%</div>
              <div style="background:var(--bg-deep);border-radius:999px;height:6px;margin:10px 0;overflow:hidden;">
              ...
              <div style="font-size:11px;color:var(--text-3);line-height:1.5;">{advice}</div>
            </div>
            """, unsafe_allow_html=True)
