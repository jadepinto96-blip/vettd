...
# Mini preview visuals per module (names/copy come from utils.modules — single source of truth)
_mod_vis = {
    "forecast": ('<div style="text-align:center;"><div class="disp" style="font-size:32px;font-weight:800;color:#F5A623;">₹23.5K</div>'
        '<div style="font-size:11px;color:var(--text-3);">earned media value</div>'
        '<div style="margin-top:10px;height:6px;background:var(--surface-inset);border-radius:999px;"><div style="width:78%;height:100%;border-radius:999px;background:linear-gradient(90deg,#7C3AED,#22D3EE);"></div></div></div>'),
    "shield": ('<div style="display:flex;justify-content:space-between;align-items:center;"><div><div class="disp" style="font-size:28px;font-weight:800;color:#34D399;">88</div><div style="font-size:10px;color:var(--text-3);">safety score</div></div>'
        '<span style="font-size:11px;font-weight:700;color:#34D399;background:rgba(52,211,153,.12);border:1px solid rgba(52,211,153,.3);padding:4px 12px;border-radius:999px;">Go</span></div>'),
    "audience": ('<div style="display:flex;align-items:center;gap:12px;"><div class="disp" style="font-size:30px;font-weight:800;color:#22D3EE;">82%</div>'
        '<div style="flex:1;"><div style="font-size:10px;color:var(--text-3);">real audience · 68% true-match</div>'
        '<div style="margin-top:6px;display:flex;height:6px;border-radius:999px;overflow:hidden;"><div style="width:82%;background:#22D3EE;"></div><div style="width:18%;background:var(--surface-inset);"></div></div></div></div>'),
    "benchmark": ('<div style="display:flex;align-items:flex-end;gap:5px;height:52px;">'
        + "".join(f'<div style="flex:1;background:{c};border-radius:4px 4px 0 0;height:{h}%;"></div>' for c,h in [('#2A2A48',40),('#2A2A48',55),('#60A5FA',88),('#2A2A48',50),('#2A2A48',35)])
        + '</div><div style="font-size:10px;color:var(--text-3);margin-top:6px;">76th percentile in category</div>'),
    "pulse": ('<div style="display:flex;align-items:flex-end;gap:6px;height:52px;">'
        + "".join(f'<div style="flex:1;background:{c};border-radius:4px 4px 0 0;height:{h}%;"></div>' for c,h in [('#34D399',80),('#60A5FA',45),('#EF4444',25),('#34D399',65),('#60A5FA',55),('#34D399',72)])
        + '</div><div style="font-size:10px;color:var(--text-3);margin-top:6px;">positive · neutral · negative</div>'),
}
mod_cards = "".join([
    f'<a href="/Feature?m={m["key"]}" target="_self" class="lift" style="text-decoration:none;background:#0D0D14;border:1px solid var(--border);border-radius:20px;padding:1.4rem;display:flex;flex-direction:column;gap:.9rem;"'
    f' onmouseover="this.style.borderColor=\'{m["color"]}55\'" onmouseout="this.style.borderColor=\'var(--border)\'">'
    f'<div style="background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:1rem;min-height:92px;display:flex;flex-direction:column;justify-content:center;">{_mod_vis[m["key"]]}</div>'
    f'<div><div style="font-size:10px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:{m["color"]};margin-bottom:2px;">Module</div>'
    f'<div class="disp" style="font-size:18px;font-weight:700;color:var(--text-1);">{m["name"]}</div></div>'
    f'<div style="font-size:12.5px;color:#7A7A98;line-height:1.6;flex:1;">{m["short"]}</div>'
    f'<div style="font-size:13px;font-weight:600;color:{m["color"]};">Learn more →</div></a>'
    for m in MODULES
])
st.markdown(f"""
<section style="padding:3.5rem 3rem;position:relative;overflow:hidden;z-index:2;max-width:1120px;margin:0 auto;">
...
<p style="font-size:16px;color:#7A7A98;max-width:620px;margin:1.25rem auto 0;line-height:1.7;">Enterprise isn't one-size-fits-all. Pick the advanced modules that match your goals — ROI forecasting, brand safety, audience credibility, competitive benchmarking, or sentiment — and we tailor the plan around them.</p>
</div>
<div class="reveal d1" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:1.1rem;margin-top:2.5rem;">{mod_cards}</div>
</section>
""", unsafe_allow_html=True)
